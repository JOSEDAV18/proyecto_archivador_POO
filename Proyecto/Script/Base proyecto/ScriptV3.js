...
class Laboratorio {
    ...
    obtenerTipoExamenMasRentable() {
        let ingresosPorTipo = {};
        for (let prueba of this.pruebas) {
            ingresosPorTipo[prueba.tipo] = (ingresosPorTipo[prueba.tipo] || 0) + prueba.calcularCostoTotal();
        }
        let tipoMasRentable = null;
        let ingresosMasAltos = 0;
        for (let tipoExamen in ingresosPorTipo) {
            if (ingresosPorTipo[tipoExamen] > ingresosMasAltos) {
                ingresosMasAltos = ingresosPorTipo[tipoExamen];
                tipoMasRentable = tipoExamen;
            }
        }
        return tipoMasRentable;
    }
}
